...
export default function DischargeFlow({ doc }: ParadigmProps) {
  ...
  useEffect(() => {
    if (!hydratedRef.current || runtimeLoading || runtimeValuesLoading || runtimeError || locked) {
  ...
  }, [autoGenerateContextKey, bodySections, locked, runtimeError, runtimeLoading, runtimeValuesLoading]);
          ...
          {runtimeLoading || runtimeError ? (
            <div className="h-full flex flex-col justify-center items-center p-6 text-center bg-[#F8FAFC]">
              ...
              <h3 className="text-sm font-bold text-slate-800 mt-3">
                {runtimeLoading ? '正在生成出院记录中，请稍后' : '出院记录配置不可用'}
              </h3>
              <p className="text-xs text-slate-400 mt-1">
                {runtimeLoading ? '正在读取模板配置并生成字段内容。' : runtimeError}
              </p>
              {!runtimeLoading && (
                <button
                  onClick={() => {
